 ...
 * @return
 */
public class SVNPackageByDirWindows {
	...
	private static Logger logger = Logger.getLogger(SVNPackageByDirWindows.class);
	...
	public static void main(String[] args) throws Exception {
		getWhlFile("E:\\EpointDM\\AI\\模型归档\\智能客服\\RASA_NEW");
		try {
				...
				try {
					...
					for (int i = 0; i < fileList.length; i++) {
						...
						if (!localversion.equals(svnversion)) {
							...
							Process p = Runtime.getRuntime().exec("cmd /c start /wait  svn update " + fileList[i]);
							p.waitFor();
							...
							copyFile(tooladdr + "tool.py", fileList[i].toString() + "\\tool.py");
							if (!new File(fileList[i].toString() + "\\打包.bat").exists()) {
								copyFile(tooladdr + "打包.bat", fileList[i].toString() + "\\打包.bat");
							}
							...
							Process p1 = Runtime.getRuntime()
							                    .exec(fileList[i].toString() + "\\打包.bat", null,
							...
							if (!whlfile.equals("")) {
								...
								copyFile(fileList[i].toString() + "\\target.zip",
								         packageaddr + projectName + "\\"
										         ...
										         "version") + "_win.zip");
								System.out.println("拷贝==" + fileList[i].toString() + "\\target.zip" + "==到目的==" + packageaddr + projectName + "\\"
										                   + fileList[i].toString().substring((fileList[i].toString().lastIndexOf("\\") + 1), fileList[i].toString().length())
										                   + "_"
										                   + getVersion(
										fileList[i].toString() + "\\packageinfo.json",
							...
							deleteDir(new File(fileList[i].toString() + "\\tool.py"));
							deleteDir(new File(fileList[i].toString() + "\\打包.bat"));
	...
	public static String getConfigValue(String fileName, String configname) {
		...
		try {
			...
			prop.load(is);
			result = prop.getProperty(configname);
	 ...
	 * @params [path]
	 */
	private static String dearWith(String path) {
		if (path.indexOf("%") != -1) {
			try {
	...
	public static String getClassesPath() {
		...
		URL classPathUrl = SVNPackageByDirWindows.class.getResource("/");
		// linux下面的服务程序运行时这个为null,加入一个处理逻辑
		if (classPathUrl == null) {
			classesPath = getPath() + "bin/";
		} else {
			String path = classPathUrl.getFile();
			// mvn的不需要处理了,edit by ko 2016-1-22
			if (path.indexOf(MVN_TAG) < 0) {
				classesPath = dearWith(classesPath);
			} else {
				classesPath = path;
			}
		}
		if (!classesPath.equals("") && classesPath.indexOf("%") != -1) {
			try {
	...
	private static String getPath() {
		String result = null;
		URL url = SVNPackageByDirWindows.class.getResource(SVNPackageByDirWindows.class.getSimpleName() + ".class");
